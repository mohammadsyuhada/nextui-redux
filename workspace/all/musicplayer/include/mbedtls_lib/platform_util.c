 ...
 * platform and needs.
 */
#if !defined(MBEDTLS_PLATFORM_HAS_EXPLICIT_BZERO) && !(defined(__STDC_LIB_EXT1__) && !defined(__IAR_SYSTEMS_ICC__)) && !defined(_WIN32)
...
#include <time.h>
						 ...
						 defined(__unix) || defined(__unix__) || (defined(__APPLE__) && defined(__MACH__)) || defined(__midipix__))
#include <unistd.h>
...
mbedtls_ms_time_t mbedtls_ms_time(void) {
	...
	current_ms = ((mbedtls_ms_time_t)ct.dwLowDateTime +
				  ((mbedtls_ms_time_t)(ct.dwHighDateTime) << 32LL)) /
				 10000;
	return current_ms;
}
#else
