	...
	*olen = 0;
	...
	if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_ECB) {
		...
		*olen = ilen;
		...
		if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->ecb_func(ctx->cipher_ctx, ctx->operation, input, output))) {
			return ret;
		}
	...
	if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_CBC) {
		...
		if (0 != ctx->unprocessed_len) {
			...
			if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->cbc_func(ctx->cipher_ctx, ctx->operation, block_size, ctx->iv, ctx->unprocessed_data, output))) {
				return ret;
			}
         ...
         */
		if (ilen) {
			if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->cbc_func(ctx->cipher_ctx, ctx->operation, ilen, ctx->iv, input, output))) {
				return ret;
			}
	...
	if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_CFB) {
		if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->cfb_func(ctx->cipher_ctx, ctx->operation, ilen, &ctx->unprocessed_len, ctx->iv, input, output))) {
			return ret;
		}
	...
	if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_OFB) {
		if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->ofb_func(ctx->cipher_ctx, ilen, &ctx->unprocessed_len, ctx->iv, input, output))) {
			return ret;
		}
	...
	if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_CTR) {
		if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->ctr_func(ctx->cipher_ctx, ilen, &ctx->unprocessed_len, ctx->iv, ctx->unprocessed_data, input, output))) {
			return ret;
		}
	...
	if (((mbedtls_cipher_mode_t)ctx->cipher_info->mode) == MBEDTLS_MODE_XTS) {
		...
		ret = mbedtls_cipher_get_base(ctx->cipher_info)->xts_func(ctx->cipher_ctx, ctx->operation, ilen, ctx->iv, input, output);
		if (ret != 0) {
			return ret;
		}
...
int mbedtls_cipher_finish_padded(mbedtls_cipher_context_t* ctx,
	...
	*invalid_padding = 0;
	...
	if (MBEDTLS_MODE_CBC == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) {
		...
		/* cipher block */
		if (0 != (ret = mbedtls_cipher_get_base(ctx->cipher_info)->cbc_func(ctx->cipher_ctx, ctx->operation, mbedtls_cipher_get_block_size(ctx), ctx->iv, ctx->unprocessed_data, output))) {
			return ret;
		}
...
int mbedtls_cipher_auth_encrypt_ext(mbedtls_cipher_context_t* ctx,
	...
	if (
		...
		mbedtls_nist_kw_mode_t mode =
			(MBEDTLS_MODE_KW == ((mbedtls_cipher_mode_t)ctx->cipher_info->mode)) ? MBEDTLS_KW_MODE_KW : MBEDTLS_KW_MODE_KWP;
