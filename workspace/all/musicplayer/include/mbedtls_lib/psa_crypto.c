...
static psa_status_t psa_verify_internal(mbedtls_svc_key_id_t key,
	...
	status = psa_get_and_lock_key_slot_with_policy(
		key, &slot,
		input_is_message ? PSA_KEY_USAGE_VERIFY_MESSAGE : PSA_KEY_USAGE_VERIFY_HASH,
		alg);
...
static psa_status_t psa_tls12_prf_psk_to_ms_set_key(
	...
	size_t data_length) {
	psa_status_t status;
	const size_t pms_len = (prf->state == PSA_TLS12_PRF_STATE_OTHER_KEY_SET ? 4 + data_length + prf->other_secret_length : 4 + 2 * data_length);
...
static psa_crypto_driver_pake_step_t convert_jpake_computation_stage_to_driver_step(
	...
	if (stage->round == PSA_JPAKE_FIRST) {
		...
		key_share_step = (stage->io_mode == PSA_JPAKE_OUTPUT) ? PSA_JPAKE_X2S_STEP_KEY_SHARE : PSA_JPAKE_X4S_STEP_KEY_SHARE;
	} else {
